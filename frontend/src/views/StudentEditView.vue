  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8">
    <div class="container mx-auto px-4 max-w-4xl">
      ...
      <div class="text-center mb-10">
        <h1 class="text-4xl font-bold text-gray-800 mb-3">編輯學生資料</h1>
        <p class="text-gray-600">修改學生詳細資訊，所有標記 * 的欄位為必填</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div v-else-if="student" class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          ...
          <!-- 錯誤訊息 -->
          <div v-else-if="errorMessage" class="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl flex items-start">
          ...
          <!-- 成功訊息 -->
          <div v-else-if="successMessage" class="mb-6 p-4 bg-green-50 border border-green-200 text-green-700 rounded-xl flex items-start">
            <svg class="w-5 h-5 mr-3 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <p class="font-medium">更新成功</p>
              <p>{{ successMessage }}</p>
            </div>
          </div>
          ...
          <form v-else @submit.prevent="submitForm" class="space-y-6">
            ...
            <!-- 更新時間 -->
            <div class="bg-gray-50 p-4 rounded-lg">
              <p class="text-sm text-gray-500">最後更新時間</p>
              <p class="font-medium">{{ formatDate(student.updated_at) }}</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          ...
          <div class="space-y-6">
            ...
            <div class="p-4 bg-gray-50 rounded-lg">
              <p class="text-sm text-gray-500 mb-1">建立時間</p>
              <p class="text-lg font-semibold text-gray-800">{{ formatDate(student?.created_at) || '尚未建立' }}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
              <p class="text-sm text-gray-500 mb-1">最後更新時間</p>
              <p class="text-lg font-semibold text-gray-800">{{ formatDate(student?.updated_at) || '尚未更新' }}</p>
            </div>
          </div>
          <div class="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div class="flex items-start">
              ...
              <p class="text-sm text-blue-700">確認所有資料無誤後，點擊「更新學生」按鈕完成修改。</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="mt-4 text-gray-600">找不到學生資料</p>
        <router-link to="/students" class="mt-4 inline-block text-blue-600 hover:text-blue-800">
          返回學生列表
        </router-link>
      </div>
    </div>
...
const loadStudent = async () => {
  ...
  try {
    ...
    if (!student.value) {
      errorMessage.value = '找不到該學生資料'
    }
  } catch (error) {
    errorMessage.value = '載入學生資料失敗'
  } finally {
    loading.value = false
  }
}
...
const submitForm = async () => {
  ...
  const result = await store.updateStudent(student.value.id, form)
